...
OUTPUT_TRAIN_DATASET = '../database/train_B_text_processed_embeddings_oai.csv'
TEST_DATASET = '../database/test_B_text.csv'
OUTPUT_TEST_DATASET = '../database/test_B_text_processed_embeddings_oai.csv'
...
def embed(input):
    print(input)
    x = get_embedding(input, model='text-embedding-ada-002')
    return np.array(x)
